...
export default function Projects() {
    ...
    if (state) {
      setTimeout(() => {
        Flip.from(state, {
          ...
          absolute: true,
          onEnter: (elements) =>
            gsap.fromTo(elements, { opacity: 0, scale: 0.95 }, { opacity: 1, scale: 1, duration: 0.4 }),
          onLeave: (elements) =>
  ...
  const handleFileSelect = (section, projectIdx, fileIdx) => {
    setOpenFolders((prev) => ({
      ...prev,
      [section]: { ...prev[section], [projectIdx]: true },
    }));
  ...
  const handleFileHover = (enter, element) => {
    if (!element.classList.contains("active")) {
      gsap.to(element, {
        ...
        color: enter ? "var(--primary-color)" : "var(--lightest-text)",
        duration: 0.2,
      });
    }
  };
    ...
    sectionKeys.forEach((section) => {
      ...
      if (selectedFile?.type === "code" && contentRefs.current[section]?.current) {
        ...
        if (codeBlocks.length) {
          gsap.fromTo(codeBlocks, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.5, stagger: 0.05, ease: "power2.out" });
        }
      }
    });
  }, [selected]);
                  ...
                  <div className="directory-sidebar">
                    <ul className="directory-tree">
                      {projects.map((project, idx) => {
                        ...
                        return (
                          <li key={idx} className="project-item" data-project-type={isSqlProject ? "sql" : "other"}>
                            <div
                            ...
                            >
                              ...
                              {isOpen ? (
                                <VscFolderOpened
                                  ...
                                  style={{
                                    marginRight: 6,
                                    color: isActive ? "var(--primary-color)" : "#8da1b9",
                                    fontSize: 15,
                                    flexShrink: 0,
                                  }}
                                />
                                ...
                                return (
                                  <li key={fIdx} className="file-item">
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        gsap.fromTo(e.currentTarget, { scale: 0.98 }, { scale: 1, duration: 0.2, ease: "back.out(1.5)" });
