...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    sectionKeys.forEach(section => {
      ...
      setOpenFolders(prev => ({ ...prev, [section]: initialOpenState }));
      setActiveFolders(prev => ({ ...prev, [section]: 0 }));
    });
  }, [sectionKeys]);
  ...
  const animateFolderToggle = (section, idx, isOpen) => {
    ...
    if (isOpen) {
      ...
      gsap.set(fileItems, { opacity: 0, y: 30 });
      gsap.to(fileItems, { opacity: 1, y: 0, duration: 0.8, stagger: 0.08, ease: "power2.out" });
      gsap.to(folderEl.querySelector('.folder-icon'), { color: "var(--green-bright)", duration: 0.3 });
      gsap.to(chevron, { rotate: 90, duration: 0.3 });
    } else {
      gsap.to(fileItems, {
        opacity: 0,
        y: 20,
      ...
      gsap.to(folderEl.querySelector('.folder-icon'), { color: "#8da1b9", duration: 0.3 });
  ...
  const handleFolderToggle = (section, idx) => {
    ...
    setActiveFolders(prev => ({ ...prev, [section]: idx }));

    const isCurrentlyOpen = openFolders[section]?.[idx];
    const readmeIdx = getReadmeIdx(projectsData[section]?.[idx]?.files || []);

    if (!isCurrentlyOpen) {
      gsap.to(foldersRef.current[section][idx], { backgroundColor: "rgba(100, 217, 138, 0.1)", duration: 0.3, ease: "power2.out" });
      setOpenFolders(prev => ({ ...prev, [section]: { ...prev[section], [idx]: true } }));
      ...
      gsap.to(foldersRef.current[section][idx], { backgroundColor: "transparent", duration: 0.3, ease: "power2.in" });
      setTimeout(() => {
        setOpenFolders(prev => ({ ...prev, [section]: { ...prev[section], [idx]: false } }));
      }, 300);
    }
  ...
  const handleFileSelect = (section, projectIdx, fileIdx) => {
    setOpenFolders(prev => ({ ...prev, [section]: { ...prev[section], [projectIdx]: true } }));
    setActiveFolders(prev => ({ ...prev, [section]: projectIdx }));

    const contentElement = contentRefs.current[section]?.current;
    if (!contentElement) {
      setSelected(prev => ({ ...prev, [section]: { projectIdx, fileIdx } }));
      return;
    }
  ...
  return (
    <section id="projects" ref={projectsRef}>
      ...
      {sectionKeys.map((section, sectionIndex) => {
        ...
        const { projectIdx = 0, fileIdx = 0 } = selected[section] || {};
        const selectedProject = projects[projectIdx] || { files: [], name: "", image: "", tags: [] };
        const selectedFile = selectedProject.files[fileIdx] || { type: "info", content: "" };
        const activeFolder = activeFolders[section] || 0;

        return (
          <div key={section} ref={el => sectionsRef.current[sectionIndex] = el} className="projects-section-container">
                ...
                <div className="directory-box">
                        ...
                        return (
                          <li key={idx} className="project-item">
                            <div
                              ...
                              onMouseEnter={(e) => {
                                if (!isActive) gsap.to(e.currentTarget, { backgroundColor: "rgba(100, 217, 138, 0.05)", duration: 0.2 });
                              }}
                              onMouseLeave={(e) => {
                                if (!isActive) gsap.to(e.currentTarget, { backgroundColor: "transparent", duration: 0.2 });
                              }}
                            >
                              <VscChevronRight
                                className="chevron-icon"
                                style={{ marginRight: 2, fontSize: 15, flexShrink: 0, transform: isOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}
                              />
                              {isOpen
                                ? <VscFolderOpened className="folder-icon" style={{ marginRight: 6, color: isActive ? "var(--green-bright)" : "#8da1b9", fontSize: 15, flexShrink: 0 }} />
                                : <VscFolder className="folder-icon" style={{ marginRight: 6, color: isActive ? "var(--green-bright)" : "#8da1b9", fontSize: 15, flexShrink: 0 }} />
                              }
                                ...
                                return (
                                  <li key={fIdx} className="file-item">
                                    <button
                                      ...
                                      onClick={(e) => { e.stopPropagation(); handleFileSelect(section, idx, fIdx); }}
                  ...
                  <div className="directory-content" ref={contentRefs.current[section]}>
                    {selectedFile.type === "info" ? (
                      <div className="project-info-container" style={{ backgroundImage: `url(${selectedProject.image})`, backgroundSize: 'cover', backgroundPosition: 'center', position: 'relative' }}>
                          ...
                          <div className="project-content">
                            ...
                            <div className="project-meta">
                              <ul className="project-tags">
                                {selectedProject.tags?.map((tag, i) => <li key={i} className="tag-pill">{tag}</li>)}
                              </ul>
                              <div className="project-links">
                                {selectedProject.dataset && <a href={selectedProject.dataset} target="_blank" rel="noopener noreferrer" title="View Dataset" className="icon-link dataset-link"><BsDatabase /></a>}
                                {selectedProject.medium && <a href={selectedProject.medium} target="_blank" rel="noopener noreferrer" title="View Article" className="icon-link"><Icon name="Medium" /></a>}
                                {selectedProject.github && <a href={selectedProject.github} target="_blank" rel="noopener noreferrer" title="GitHub" className="icon-link github-link"><Icon name="GitHub" /></a>}
                                {selectedProject.tableau && <a href={selectedProject.tableau} target="_blank" rel="noopener noreferrer" title="View Dashboard" className="icon-link tableau-link"><Icon name="Tableau" /></a>}
                              </div>
                            </div>
                          </div>
                        </div>
